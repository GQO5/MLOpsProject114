...
Student s253814 was in charge of setting up the initial framework for training the model and making a series of invoke commands for CLI tasks to train the model. Also handled setting up the Google Cloud Storage bucket for group 114, integrating it with DVC for version-controlled data storage, and configuring the Vertex AI custom jobs to run our training container while ensuring results were logged to Weights & Biases for tracking. Also used GitHub Copilot with Grok Code Fast 1 to assist with complex error messages and compatibility issues between packages
