...
def load_food_nutrients_dataset(
    ...
    # This avoids Windows confusing "hf://" URLs with local drive paths.
...
def build_manifest_from_hf(ds_dict: DatasetDict, cfg: PreprocessConfig) -> pd.DataFrame:
    ...
    """
    # Logic to handle different dataset structures
    if set(ds_dict.keys()) >= {"train", "validation", "test"}:
        ...
        # Case: Single split exists (e.g. 'test') or non-standard splits.
        # We merge everything and create our own splits.
...
def export_images_and_update_manifest(
    ...
    # Collect all images into a single list to match the concatenated manifest
    images = []
    ...
    for i, img in enumerate(images):
        ...
        # Save only if it doesn't exist to save time on re-runs (optional)
